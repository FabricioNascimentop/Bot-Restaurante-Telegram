...
ProductsDF = pandas.DataFrame(pandas.read_excel("C:\PyProjects\Bot Telegram\Excel files\Products Exemple.xlsx"))
pedidos = []
...
def procurador(msg):
    ...
    for l in range(0, len(lista)):
        for c in range(0, ProductsDF.shape[0]):
            if str(msg.text).lower() == str(ProductsDF[lista[l]][c]).lower():
                return True
...
def cadastro_pedido(msg):
    bot.send_message(msg.chat.id,'your order has already been deleted, do you want to go back to /menu? Just click on the purple link')
